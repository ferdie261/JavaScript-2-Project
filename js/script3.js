...
document.addEventListener('DOMContentLoaded', () => {
    ...
    window.location.href = `checkout.html?product=Damascus%20Knife&qty=${quantity}&total=${total}`;
...
let images = [
    'images/damascus.png',
    'images/damascus1.png',
    'images/damascus2.png'
];
...
addtoCart.addEventListener('click', function () {
    ...
    if (quantity > 0) {
        let existingItem = document.getElementById('knifeItem');
        if (existingItem) {
            existingItem.textContent = `${quantity}x Damascus Knife = ₱${(quantity*unitPrice).toFixed(2)}`;
        } else {
            const item = document.createElement('p');
            item.id = 'knifeItem';
            item.textContent = `${quantity}x Damascus Knife = ₱${(quantity*unitPrice).toFixed(2)}`;
            itemList.appendChild(item);
