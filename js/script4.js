...
document.addEventListener('DOMContentLoaded', () => {
    ...
    window.location.href = `checkout.html?product=Petty%20Knife&qty=${quantity}&total=${total}`;
...
let images = [
    'images/fishknife1.png',
    'images/fishknife2.png',
    'images/fishknife3.png'
];
...
addtoCart.addEventListener('click', function () {
    ...
    if (quantity > 0) {
        let existingItem = document.getElementById('knifeItem');
        if (existingItem) {
            existingItem.textContent = `${quantity}x Petty Knife = ₱${(quantity*unitPrice).toFixed(2)}`;
        } else {
            const item = document.createElement('p');
            item.id = 'knifeItem';
            item.textContent = `${quantity}x Petty Knife = ₱${(quantity*unitPrice).toFixed(2)}`;
            itemList.appendChild(item);
